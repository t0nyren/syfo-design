...
const t0=(await p.innerText('body')).replace(/\n{2,}/g,' ');
console.log('Guide added (system msg)?', t0.includes('Guide was added')||t0.includes('Guide'));
const comp = p.locator('textarea[placeholder*="Message"]').first();
await comp.click(); await p.waitForTimeout(400);
await p.keyboard.type('@Guide',{delay:60}); await p.waitForTimeout(1500);
...
console.log('posted?', t.includes('starter checklist'));
console.log('task#:', (t.match(/Task #\d+/)||[''])[0]);
await b.close();
