...
function AgentProfile({ member }) {
  ...
  const tabs = [
    { id: 'dms',         label: 'Agent DMs',   icon: 'message-square' },
    { id: 'profile',     label: 'Profile',     icon: 'info' },
    { id: 'workspace',   label: 'Workspace',   icon: 'hash' },
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', minHeight: 0, overflowY: 'auto' }}>
      ...
      <div style={{ padding: '24px 32px 0' }}>
        <div style={{ display: 'flex', alignItems: 'flex-start', gap: 18 }}>
          ...
          <div style={{ display: 'flex', gap: 8 }}>
            ...
            <Button variant="secondary" icon title="Pause"><Icon name="minus" size={14} /></Button>
            <Button variant="secondary" icon title="Restart"><Icon name="activity" size={14} /></Button>
          </div>
        </div>
      </div>
      ...
      <div style={{ padding: '24px 32px', maxWidth: tab === 'activity' ? 920 : 720 }}>
        ...
        {tab !== 'profile' && tab !== 'activity' && <EmptyState title="Nothing here yet" body="This view is part of the design system; live data populates once the workspace is connected." />}
      </div>
    </div>
  );
}
...
function AgentProfileBody({ member }) {
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
      <Field label="Display name" value={member.name} />
      <Field label="Description" value={member.desc} />
      ...
      <div>
        ...
        <div className="card" style={{ padding: '14px 16px', display: 'flex', flexDirection: 'column', gap: 12 }}>
          <Row k="Computer" v={<><PresenceDot presence="online" /> <span style={{ fontFamily: 'var(--font-mono)' }}>breeze-cobra-99</span> <span className="meta">· Connected · daemon v0.51.1</span></>} />
          <Row k="Created" v={<span className="mono" style={{ fontSize: 13, color: 'var(--fg-1)' }}>May 13, 2026</span>} />
          <Row k="Creator" v={<span style={{ display: 'inline-flex', alignItems: 'center', gap: 8 }}><Avatar name="tonyren" size="sm" /> <span>tonyren</span> <span className="meta">@tonyren</span></span>} />
        </div>
      </div>
      ...
      <div>
        ...
        <div className="card" style={{ padding: '14px 16px', display: 'flex', flexDirection: 'column', gap: 16 }}>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 16 }}>
            <KVStack label="Runtime"><Chip tone="info">{member.runtime}</Chip></KVStack>
            <KVStack label="Model"><Chip tone="accent">{member.model}</Chip></KVStack>
            <KVStack label="Reasoning"><Chip>{member.reasoning}</Chip></KVStack>
          </div>
        </div>
      </div>
      ...
      <div>
        ...
        <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
          {['Bash', 'Python', 'Git', 'Read code', 'Edit code', 'Run tests', 'Deploy'].map(s => (
            <Chip key={s}>{s}</Chip>
          ))}
        </div>
      </div>
...
function HumanProfileBody({ member }) {
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
      <Field label="Display name" value={member.name} />
      <Field label="Handle" value={member.handle} />
