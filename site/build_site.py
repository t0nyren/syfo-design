...
CASES = {
 ...
 ("消费电商 · 达人运营", "把最耗人的达人建联，改造成 Agent 驱动的流水线",
  "人一句话下达业务目标，Agent 完成目标校验、周拆分、发布建联任务；没有 API 的反爬环节生成人工待办推送 IM，人回传文件后 Agent 接管其后所有分析与流转，配套系统三天上线。",
...
CASE_GROUPS = {
 "zh": [
  {"id": "consumer", "label": "消费 / 电商", "eyebrow": "行业案例",
   "p": "品牌与电商团队在 Syfo 上的真实场景：一家功能性食品品牌的七个运营场景（组织接入、达人建联、广告投放、经营日报、用户研究、新品调研、内部系统），以及一家女鞋品牌的经营诊断与新品评审。案例均已脱敏。",
  ...
  {"id": "finance", "label": "金融 / 投资", "eyebrow": "行业案例",
   "p": "一家小型投资机构在 Syfo 上的完整用法：两支策略分别由 Agent 出任 CIO 与交易员，人只负责提假设、问问题、拍板。从一只系统化私募的日常投研，到一个宏观策略从设计走到实盘，再到研究纪律、独立验证、风控执行与自建组合管理平台。案例均已脱敏。",
...
DETAILS = {
 "zh": {
  ...
  # ══ 行业案例 · 品牌 / 电商（brand-*，脱敏：某北美功能性食品品牌，多国站点达人带货 + 投流）══
  "brand-onboard":{
  ...
  "brand-creator":{
   ...
   "want":"把整条建联链路改造成「Agent 决策 + 人类执行反爬环节 + 系统自动监控」的闭环：人用一句话下达业务目标（「这个月这个产品要 N 条视频」），Agent 完成目标校验、周拆分、发布建联任务；选人工具没有 API 的环节生成「人工待办」并推送 IM 通知，人导出文件回传后 Agent 接管其后所有分析与流转。",
  ...
  "brand-research":{
   ...
   "agents":[
     ("@新品调研","外部信号扫描","按国家扫新品与热卖榜：品类、功效方向、剂型包装、价格带、竞品、内容钩子。"),
  ...
  "brand-health":{
   ...
   "briefing":"我们要把「舒适度提升」从倡议变成经营机制。规则：\n· 评级必须绑定动作：每一级对应放大 / 修复 / 再验证 / 降权 / 淘汰，不只是打标签。\n· 客诉必须拆根因：楦型、尺码、材质、预期、批次、物流，不同根因对应不同部门和周期。\n· 滞后指标（长周期回购率）必须配前置指标（首穿负反馈、短周期退款、复购）。\n· 高销售高客诉的款要设止损闸门：投放闸、商品闸、供应链闸。\n· 每个问题款一张作战卡：Owner、截止日期、验证指标，缺一不进优化池。",
   "workflow":[
  ...
  "brand-npd":{
   ...
   "workflow":[
     ...
     ("首轮判断","Agent 几分钟通读，给方向对比矩阵 + 关键修正信号（用户要的不是更夸张的设计，而是舒适与造型的平衡）。"),
     ("周期与测算","倒排开发周期、点出锁款闸门时间点；商业测算给保守 / 基准 / 乐观三档情景和判断标准。"),
     ("企划二轮评审","产品经理企划出来后再评：认可方向，同时指出企划未解决的核心工程矛盾（该品类真正需要的是分区设计而非单一指标拉满）。"),
  ...
  # ── 行业案例 · 金融 / 投资 ──
  "fin-allweather":{
   "sub":"团队想把一个宏观资产配置策略做成正式产品：回测要能复现、口径要对齐基金合同、上线后每天要能对账——传统上这是一支投研加中后台团队几个月的事。",
   "meta":[("角色","2 位人类 + 3 个 Agent"),("起始频道","#策略产品 · #研究系列 · #实盘运维"),("周期","设计到实盘约三周"),("数据","期货行情 · 券商结算单")],
   "want":"让 CIO Agent 和工程 Agent 把一个全天候式宏观策略从想法推进到实盘：写死《可复现说明书》、当天完成十二年历史回测、系统性扫描参数档位、把全套材料对齐基金合同的合规口径，实盘之后接管日度运维。人只提假设、问问题、拍板。",
   ...
   "tips":["先写死《可复现说明书》再动手——之后所有口径争议都回到这份文档裁决。",
           "参数决策留给人：Agent 的职责是把每档的收益、回撤、杠杆需求摆到一页纸上。",
           "上线不是终点：日度对账、缺产物报警、换月 playbook 才是实盘的日常。"],
  },
  "fin-discipline":{
