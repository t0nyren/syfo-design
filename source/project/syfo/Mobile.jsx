  ...
  return (
    <div className="syfo-phone">
      ...
      <nav className="syfo-phone-tabbar">
        <MobileTabBtn label="Home"     icon="message-square" active={!top && tab === 'home'}     onClick={() => { setStack([]); setTab('home'); }} />
        <MobileTabBtn label="Tasks"    icon="list-todo"      active={!top && tab === 'tasks'}    onClick={() => { setStack([]); setTab('tasks'); }} />
        <MobileTabBtn label="Members"  icon="users"          active={!top && tab === 'members'}  onClick={() => { setStack([]); setTab('members'); }} />
        <MobileTabBtn label="Settings" icon="settings"       active={!top && tab === 'settings'} onClick={() => { setStack([]); setTab('settings'); }} />
      </nav>
    </div>
  );
}
      ...
      <>
        ...
        <div className="syfo-phone-body">
          <MobileSettingsGroup title="Server">
            ...
            <MobileSettingsRow icon="users"   label="Members" trailing={`${MEMBERS.length}`} onClick={() => setSection('members')} />
    ...
    <>
      ...
      <div className="syfo-phone-body">
        ...
        <article className="syfo-phone-card" style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
          ...
          <MobileKV k="Created" v={<span style={{ fontFamily: 'var(--font-mono)' }}>{c.created}</span>} />
        </article>
